...
The first script above runs indefinitely and pulls and logs the temp every 1hr and reports to ntfy.sh every 24 hrs. It also issues a warning if the temp drops below 50*F (you can adjust this). That warning will come every hour until the temp increases above 50*F. on boot-up it also gives a status ping to the channel with the current temp, to let you know it's alive and well without waiting 24 hrs.

the second script is a listener. it must share the same topic as the first, and it listens to the topic and if it gets the command `temp now` it responds with the current temp. useful if you want a quick update of how things are.
