...
anyway, the bash command `usb_temp` runs hid-query and ask the thermometer for temp in some hex blob and returns it in \({}^{\circ }\)C, a la:
...
The first script above runs indefinitely and pulls and logs the temp every 1hr and reports to ntfy.sh every 24 hrs. It also issues a warning if the temp drops below 50\({}^{\circ }\)F (you can adjust this). That warning will come every hour until the temp increases above 50\({}^{\circ }\)F. on boot-up it also gives a status ping to the channel with the current temp, to let you know it's alive and well without waiting 24 hrs.
